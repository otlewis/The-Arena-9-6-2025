...
const config = {
  ...
  mediasoup: {
    numWorkers: process.env.MEDIASOUP_WORKERS || os.cpus().length,
    worker: {
      rtcMinPort: 10000,
      rtcMaxPort: 10100,
      logLevel: 'warn',
...
async function getOrCreateRoom(roomId, roomType) {
  ...
  if (!room) {
    // Check if room exists on another server
    const roomInfo = await redisClient.hgetall(`room:${roomId}`);
    
    if (roomInfo && roomInfo.serverId && roomInfo.serverId !== config.serverId) {
      // Room exists on another server
      return { room: null, redirect: {
        serverIp: roomInfo.serverIp,
        serverPort: roomInfo.serverPort,
      }};
    }
    
    // Create room on this server
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', async ({ roomId, peerId, roomType, role }) => {
    try {
      const { room, redirect } = await getOrCreateRoom(roomId, roomType);
      
      if (redirect) {
        // Tell client to connect to different server
  ...
  socket.on('consume', async ({ producerId, rtpCapabilities }, callback) => {
    try {
      ...
      let transport = Array.from(peer.transports.values())
        .find(t => t.appData.consuming);
      
      if (!transport) {
        throw new Error('No consuming transport found');
      }
...
setInterval(async () => {
  try {
    await redisClient.hset(`server:${config.serverId}`, {
      ...state.metrics,
      timestamp: Date.now(),
    ...
    await redisClient.expire(`server:${config.serverId}`, 30);
  } catch (error) {
...
async function start() {
  try {
    ...
    server.listen(config.listenPort, config.listenIp, () => {
      console.log(`🚀 MediaSoup scalable server running on ${config.listenIp}:${config.listenPort}`);
