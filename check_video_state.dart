...
void main() async {
  ...
  print('🔍 Checking debate_discussion_participants...\n');
  ...
  try {
    ...
    for (var doc in participants.documents) {
      ...
      print('videoReady: ${data['videoReady'] ?? 'NOT SET'}');
      print('audioReady: ${data['audioReady'] ?? 'NOT SET'}');
      print('videoTrackSid: ${data['videoTrackSid'] ?? 'NOT SET'}');
      print('audioTrackSid: ${data['audioTrackSid'] ?? 'NOT SET'}');
      print('Joined: ${data['joinedAt']}');
      print('');
    }
  } catch (e) {
    print('❌ Error: $e');
