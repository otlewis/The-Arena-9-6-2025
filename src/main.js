...
export default async ({ req, res, log, error }) => {
  try {
    ...
    // Use provided data or defaults - FIXED UID handling
    const channelName = requestData.channelName || 'arena';
    const uid = requestData.uid !== undefined ? requestData.uid : 0; // Use 0 as default, match Flutter
    const role = requestData.role || 'subscriber'; // Default to subscriber role
