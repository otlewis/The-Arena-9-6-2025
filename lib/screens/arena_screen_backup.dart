...
class _ArenaScreenState extends State<ArenaScreen> with TickerProviderStateMixin {
  ...
  void _showJudgeSelectionModal() {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (context) => JudgeSelectionModal(
        arenaRoomId: widget.roomId,
        topic: widget.topic,
        category: widget.category,
        description: widget.description,
        onJudgesSelected: (selectedJudges) {
          AppLogger().info('Moderator selected ${selectedJudges.length} judges');
  ...
  void _showModeratorControlModal() {
    showModalBottomSheet(
      ...
      builder: (context) => ModeratorControlModal(
        ...
        onSelectJudges: _showJudgeSelectionModal,
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              // Main arena content - more compact
              Expanded(
                child: Column(
                  children: [
                    // Top Row - Debaters (40% of available space)
                    Flexible(
                      flex: 40,
                      child: Row(
                        children: [
                    ...
                    // Middle Row - Moderator (25% of available space)
                    Flexible(
                      flex: 25,
                      child: Row(
                        children: [
                    ...
                    // Bottom Row - Judges (30% of available space)
                    Flexible(
                      flex: 30,
                      child: Row(
                        children: [
                    ...
                    // Audience Display (compact and scrollable - only 5% of available space)
                    Flexible(
                      flex: 5,
                      child: _buildCompactAudienceDisplay(),
                    ),
                  ],
                ),
              ),
        ...
        children: [
          ...
          Expanded(
            child: SizedBox(
              ...
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: _audience.length > 8 ? 8 : _audience.length,
                itemBuilder: (context, index) {
                  if (index == 7 && _audience.length > 8) {
  ...
  Widget _buildControlPanel() {
    // Only show the control panel if there are relevant controls to show
    final hasControls = _isJudge || _isModerator || kDebugMode || _judgingComplete;
        ...
        child: SingleChildScrollView(
          ...
          child: Row(
            ...
            children: [
              ...
              // Debug refresh (only in debug mode)
              if (kDebugMode)
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 6),
                  child: _buildControlButton(
  ...
  void _showJudgeSelection() {
    if (_audience.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No audience members available to select as judges'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.balance, color: Colors.amber.shade700),
                  const SizedBox(width: 12),
                  const Expanded(
                    child: Text(
                      'Select Judges from Audience',
                      style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                    ),
                  ),
                  IconButton(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'Select up to 3 judges • ${_getAvailableJudgeSlots()} slots available',
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    title: Text(audienceMember.name),
                    subtitle: Text(
                      audienceMember.isAvailableAsJudge 
                          ? 'Available as judge' 
                          : 'Audience member',
                      style: TextStyle(
                        color: audienceMember.isAvailableAsJudge ? Colors.green : Colors.grey,
                      ),
                    ),
                    trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                    onTap: () => _assignJudgeFromAudience(audienceMember),
                  );
                },
              ),
            ),
...
class ModeratorControlModal extends StatelessWidget {
  ...
  final VoidCallback onSelectJudges;
