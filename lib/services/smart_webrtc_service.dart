...
class SmartWebRTCService extends ChangeNotifier {
  ...
  bool _shouldPublishMedia(String role, String roomType) {
    switch (roomType) {
      case 'arena':
        // In Arena: debaters and judges can publish
  ...
  Future<bool> testServerConnectivity(String serverUrl) async {
    try {
      debugPrint('🔍 Testing MediaSoup server connectivity...');
      ...
      testSocket.once('connect', (_) {
        debugPrint('✅ Server connectivity test successful');
  ...
  Future<void> connect(
    ...
    try {
      debugPrint('🚀 Smart WebRTC connect() called with:');
      ...
      _canPublishMedia = _shouldPublishMedia(role, roomType);
      debugPrint('🔄 Media permissions re-set after cleanup: $_canPublishMedia (role: $role, roomType: $roomType)');
      
      // Test server connectivity first
      debugPrint('🔍 Running connectivity pre-check...');
      ...
      final serverUri = serverUrl.startsWith('http') 
          ? serverUrl 
          : 'http://$serverUrl';
      debugPrint('🔌 Connecting to MediaSoup server: $serverUri');
      debugPrint('📡 Using polling-first transport for mobile compatibility...');
      ...
      debugPrint('🚪 About to join MediaSoup room...');
      await _joinRoom(room, userId, role, roomType);
      debugPrint('✅ Successfully joined MediaSoup room');
      
      // Initialize MediaSoup device AFTER joining (now room exists)
      debugPrint('📱 About to initialize MediaSoup device...');
      await _initializeDevice();
      debugPrint('✅ MediaSoup device initialized successfully');
      
      // Only initialize media if role can publish  
      debugPrint('🔍 Checking media permissions: canPublish=$_canPublishMedia, role=$role, roomType=$roomType');
      if (_canPublishMedia) {
        ...
        debugPrint('🔇 Role "$role" is receive-only - skipping media initialization');
      }
      ...
      debugPrint('❌ Smart WebRTC connection error: $e');
  ...
  Future<void> _initializeDevice() async {
    try {
      ...
      debugPrint('📡 Using stored router RTP capabilities');
      
      // Load device with router capabilities (convert Map to RtpCapabilities)
      debugPrint('📊 Router RTP capabilities type: ${_routerRtpCapabilities.runtimeType}');
      debugPrint('📊 Router RTP capabilities keys: ${_routerRtpCapabilities!.keys.toList()}');
  ...
  Future<void> _joinRoom(String room, String userId, String role, String roomType) async {
    try {
      debugPrint('🚪 Joining MediaSoup room: $room as $role in $roomType');
      ...
      _socket!.once('room-joined', (data) {
        ...
        if (data['rtpCapabilities'] != null) {
          ...
          debugPrint('📡 Stored router RTP capabilities from room-joined response');
        }
        ...
        if (data['existingProducers'] != null) {
          debugPrint('🎧 Found ${data['existingProducers'].length} existing producers');
          ...
          Future.delayed(const Duration(milliseconds: 500), () async {
            for (final producer in existingProducers) {
              debugPrint('🎧 Consuming existing producer: ${producer['producerId']} from ${producer['userId']} (${producer['role']})');
  ...
  Future<void> _createSendTransport() async {
    if (!_canPublishMedia) {
      debugPrint('⚠️ Skipping send transport - role cannot publish');
      return;
    }
    ...
    try {
      ...
      _sendTransport!.on('connect', (data) async {
        debugPrint('🔗 Send transport connect event triggered!');
        debugPrint('🔗 Transport ID: ${_sendTransport!.id}');
        debugPrint('🔗 DTLS Parameters received: ${data['dtlsParameters'] != null}');
        ...
        try {
          ...
          debugPrint('✅ Send transport connected to server');
          data['callback']();
        } catch (e) {
          debugPrint('❌ Send transport connection failed: $e');
          data['errback'](e);
        }
      });
      ...
      _sendTransport!.on('produce', (data) async {
        ...
        debugPrint('🎬 Track kind: ${data['kind']}');
        debugPrint('🎬 RTP Parameters: ${data['rtpParameters'] != null}');
        debugPrint('🎬 App Data: ${data['appData']}');
        ...
        try {
          ...
          debugPrint('✅ Producer created on server: $producerId');
          
          data['callback']({'id': producerId});
          
        } catch (e) {
          debugPrint('❌ Production failed: $e');
          data['errback'](e);
        }
      });
      ...
      if (_localStream != null) {
        ...
        if (audioTracks.isNotEmpty) {
          debugPrint('🎤 Triggering immediate audio production...');
          try {
            ...
            final producerId = response['producerId'];
            debugPrint('✅ Manual audio producer created: $producerId');
            
          } catch (e) {
            debugPrint('⚠️ Manual audio production failed: $e');
            debugPrint('📡 Will rely on transport events for production');
          }
        }
      }
  ...
  Future<void> _createRecvTransport() async {
    try {
      ...
      debugPrint('💡 Existing producers will be consumed from room-joined response');
  ...
  Future<void> _produceTracks() async {
    if (!_canPublishMedia || _localStream == null || _sendTransport == null) {
      debugPrint('⚠️ Cannot produce tracks - canPublish: $_canPublishMedia, hasStream: ${_localStream != null}, hasTransport: ${_sendTransport != null}');
      return;
    }
    
    try {
      debugPrint('🎤 Producing tracks for role: $_userRole');
      
      // Produce audio tracks
      final audioTracks = _localStream!.getAudioTracks();
      if (audioTracks.isNotEmpty) {
        debugPrint('🎤 Audio tracks available for production: ${audioTracks.length}');
        
        final audioTrack = audioTracks.first;
        debugPrint('🎤 Producing audio track: ID=${audioTrack.id}, enabled=${audioTrack.enabled}');
        
        // The transport events will handle the actual production
        // Just log that we have audio tracks ready for production
        debugPrint('🎤 Audio tracks ready for production via transport events');
        debugPrint('🎤 First audio track: ID=${audioTrack.id}, enabled=${audioTrack.enabled}');
      ...
      if (_isVideoEnabled) {
        ...
        if (videoTracks.isNotEmpty) {
          debugPrint('🎥 Would produce video track (temporarily disabled for API compatibility)');
  ...
  Future<void> _initializeMedia({required bool audioOnly}) async {
    if (!_canPublishMedia) {
      debugPrint('⚠️ Skipping media initialization - role cannot publish');
      return;
    }
    
    try {
      debugPrint('🎥 Initializing media for role: $_userRole - audioOnly: $audioOnly');
      ...
      debugPrint('✅ Media initialized for $_userRole');
  ...
  void _setupSocketListeners() {
    ...
    _socket!.on('connecting', (_) {
      debugPrint('🔄 Socket is connecting...');
    });
    
    _socket!.on('connect_error', (error) {
      debugPrint('❌ MediaSoup socket connection error: $error');
      debugPrint('❌ Error type: ${error.runtimeType}');
      debugPrint('❌ Socket ID during error: ${_socket!.id}');
      debugPrint('💡 Network troubleshooting:');
    ...
    debugPrint('🎧 Socket listeners set up complete');
    ...
    _socket!.on('role-changed', (data) {
      ...
      debugPrint('🔄 User $userId role changed to: $newRole');
    ...
    _socket!.on('screen-share-started', (data) {
      final userId = data['userId'];
      debugPrint('🖥️ User $userId started screen sharing');
      onRemoteScreenShareChanged?.call(userId, true);
    });
    
    _socket!.on('screen-share-stopped', (data) {
      final userId = data['userId'];
      debugPrint('🛑 User $userId stopped screen sharing');
  ...
  Future<void> _consumeProducer(Map<String, dynamic> data) async {
    try {
      ...
      if (_recvTransport == null) {
        debugPrint('⚠️ No receive transport available');
        return;
      }
      ...
      debugPrint('✅ Consumer created for $userId ($role): $consumerId');
  ...
  void _stopPublishing() {
    debugPrint('🛑 Stopping media publishing');
  ...
  Future<void> startScreenShare() async {
    if (!_canPublishMedia) {
      debugPrint('⚠️ Cannot start screen share - role cannot publish');
      return;
    }
  ...
  Future<void> updateRole(String newRole) async {
    ...
    debugPrint('🔄 Updating role from $_userRole to $newRole');
