...
// Socket.IO configuration optimized for Flutter client
const io = socketIo(server, {
  path: '/socket.io/',
  transports: ['polling'], // Start with polling only - no websocket upgrade
  allowEIO3: false, // Use Engine.IO v4
  pingTimeout: 60000,
  pingInterval: 25000,
  allowUpgrades: false, // Prevent websocket upgrades that cause issues
...
async function startServer() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🎯 Transport: polling-only (no websocket upgrade)`);
