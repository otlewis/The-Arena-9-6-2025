import 'dart:io';
import 'dart:convert';

Future<void> main() async {
  const apiKey = 'standard_';
  const projectId = '683a37a8003719978879';
  const databaseId = 'arena_db';

  final client = HttpClient();

  print('🔍 Checking room debater fields...');

  const roomId = 'arena_68daf2e03a04757e3b43';

  try {
    final roomRequest = await client.getUrl(
      Uri.parse('https://cloud.appwrite.io/v1/databases/$databaseId/collections/arena_rooms/documents/$roomId')
    );
    roomRequest.headers.set('X-Appwrite-Project', projectId);
    roomRequest.headers.set('X-Appwrite-Key', apiKey);

    final roomResponse = await roomRequest.close();
    final roomBody = await utf8.decodeStream(roomResponse);

    if (roomResponse.statusCode == 200) {
      final roomData = jsonDecode(roomBody);
      print('✅ Room found!');

      // Check debater fields specifically
      print('\n📋 Debater fields:');
      print('   - debater1Id: ${roomData['debater1Id']} (${roomData['debater1Id']?.runtimeType})');
      print('   - debater2Id: ${roomData['debater2Id']} (${roomData['debater2Id']?.runtimeType})');
      print('   - challengerId: ${roomData['challengerId']} (${roomData['challengerId']?.runtimeType})');
      print('   - challengedId: ${roomData['challengedId']} (${roomData['challengedId']?.runtimeType})');
      print('   - moderatorId: ${roomData['moderatorId']} (${roomData['moderatorId']?.runtimeType})');

      // Show what we would use for fallback
      final debater1 = roomData['debater1Id'] ?? roomData['moderatorId'];
      final debater2 = roomData['debater2Id'] ?? roomData['moderatorId'];
      print('\n💭 Fallback logic would use:');
      print('   - debater1Id fallback: $debater1');
      print('   - debater2Id fallback: $debater2');

    } else {
      print('❌ Failed to get room: $roomBody');
    }

  } catch (e) {
    print('❌ Error: $e');
  }

  client.close();
}