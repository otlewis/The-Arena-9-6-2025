...
console.log('=== Testing Peer Events LOCALLY ===');

// Connect two clients to LOCAL server for testing
const client1 = io('http://localhost:3001/signaling', {
  transports: ['websocket', 'polling']
});

const client2 = io('http://localhost:3001/signaling', {
  transports: ['websocket', 'polling']
});
...
client2.on('room-joined', (data) => {
  ...
  setTimeout(() => {
    console.log('--- Checking if Client2 received existing peers ---');
  }, 500);
});

client2.on('peer-joined', (data) => {
  console.log('Client2 👤 PEER-JOINED (existing peer):', data);
});
